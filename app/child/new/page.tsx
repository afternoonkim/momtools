...
export const metadata: Metadata = {
  title: "아이 정보 등록",
  description: "출산예정일 또는 출생일을 등록하고 내 아이 기준으로 필요한 정보를 확인하세요.",
    ...
    <main className="mt-page">
      <div className="mt-container max-w-xl space-y-5">
        <section className="mt-card p-5 md:p-7">
          ...
          <h1 className="mt-title-lg mt-4">{hasChildren ? "아이를 추가로 등록해 주세요" : "아이 정보를 등록해 주세요"}</h1>
          <p className="mt-text-main mt-3">
            {hasChildren
              ? "아이를 여러 명 등록하면 임신 홈과 육아 홈을 아이별로 따로 선택해 확인할 수 있어요. 가족 연결을 하면 엄마와 아빠가 같은 기록을 함께 볼 수 있어요."
              : "출산 전에는 임신 주차와 산전검진 흐름을, 출산 후에는 월령·이유식·발달 체크를 내 아이 기준으로 확인할 수 있어요. 가족 연결을 하면 다른 보호자도 같은 기록을 함께 볼 수 있어요."}
          </p>
          {hasChildren ? (
