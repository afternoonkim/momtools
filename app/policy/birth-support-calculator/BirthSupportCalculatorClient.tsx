...
import { Calculator, CheckCircle2, Landmark, WalletCards } from "lucide-react";
import {
  birthOrderLabels,
  birthSupportRegions,
      ...
      <div className="border-b border-amber-100 bg-gradient-to-br from-amber-50 via-white to-sky-50 p-6 md:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-text-sub mt-3 max-w-3xl">
              2026년 전국 공통 지원금 기준과 제주도·경남·경북·전남·전북·충남·충북·경기·강원·세종·울산·대전·광주·인천·대구·부산·서울 지역별 상세 자료를 반영했습니다. 지역과 출생 순위를 선택하면 첫만남이용권, 부모급여, 아동수당, 지자체 출산지원금을 먼저 계산하고, 가정양육수당처럼 조건에 따라 달라지는 지원은 별도로 안내합니다.
            </p>
          </div>
        ...
        <div className="space-y-5">
          <label className="block">
            ...
            <select
              ...
              className="mt-2 w-full rounded-2xl border border-amber-100 bg-white px-4 py-3 text-slate-800 outline-none transition focus:border-amber-300 focus:ring-4 focus:ring-amber-100"
            >
              {birthSupportRegions.map((region) => (
                <option key={region.regionCode} value={region.regionCode}>
                  {region.sido} {region.sigungu === "전체" ? "" : region.sigungu}
                </option>
              ))}
            </select>
          </label>
          ...
          <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 text-sm leading-7 text-slate-600">
            ...
            <p className="mt-2">
              총액은 첫만남이용권, 부모급여, 아동수당, 선택한 지역의 지자체 지원금을 합산했습니다. 부모급여는 0세 월 100만 원·1세 월 50만 원 기준이며, 아동수당은 거주 시·도에 따라 수도권 월 10만 원 또는 비수도권 월 10.5만 원 기준으로 적용됩니다. 가정양육수당은 어린이집·유치원 이용 여부에 따라 달라져 별도로 안내합니다.
            </p>
          </div>
        </div>
