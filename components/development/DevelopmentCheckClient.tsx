...
type DevelopmentRecordMap = Record<
  string,
  { status: DevelopmentStatus; note?: string | null }
>;
...
type StatusStats = Record<DevelopmentStatus, number> & {
  total: number;
  checked: number;
};
...
function StatusButton({
  ...
  return (
    <button
      ...
      data-mt-status={status}
      onClick={onClick}
      className={`mt-selectable-button rounded-full border px-2.5 py-1.5 text-[11px] font-extrabold transition active:scale-[0.98] disabled:opacity-60 ${statusClass(status, active)}`}
    >
...
export default function DevelopmentCheckClient({
  childrenOptions,
  selectedChildId,
  initialRecords,
  initialHistory,
}: DevelopmentCheckClientProps) {
  const selectedChild =
    childrenOptions.find((child) => child.id === selectedChildId) ??
    childrenOptions[0];
  const selectedIndex = childrenOptions.findIndex(
    (child) => child.id === selectedChild.id,
  );
  const selectedName = childDisplayName(
    selectedChild,
  ...
  const expectedDueDate = selectedChild.expectedDueDate
    ? parseBirthDate(selectedChild.expectedDueDate)
    : null;
  const usesCorrectedAge = Boolean(
    selectedChild.useCorrectedAge &&
    expectedDueDate &&
    expectedDueDate.getTime() > birthDate.getTime() &&
    actualMonths < 24,
  );
  const correctedMonths =
    usesCorrectedAge && expectedDueDate
      ? getMonthDiffWithDayAdjustment(expectedDueDate, today)
      : null;
  const currentMonths = correctedMonths ?? actualMonths;
  const currentAgeRangeKey = getDevelopmentAgeRangeKey(currentMonths);
  const [ageRangeKey, setAgeRangeKey] =
    useState<DevelopmentAgeRangeKey>(currentAgeRangeKey);
  ...
  const [history, setHistory] =
    useState<DevelopmentHistoryItem[]>(initialHistory);
  const [noteDrafts, setNoteDrafts] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      Object.entries(initialRecords).map(([key, value]) => [
        key,
        value.note ?? "",
      ]),
    ),
  ...
  const range =
    developmentAgeRanges.find((item) => item.key === ageRangeKey) ??
    developmentAgeRanges[0];
  const domain =
    developmentDomains.find((item) => item.key === domainKey) ??
    developmentDomains[0];
  const milestones = developmentMilestones.filter(
    (item) => item.ageRangeKey === ageRangeKey && item.domainKey === domainKey,
  );
  const ageRangeMilestones = developmentMilestones.filter(
    (item) => item.ageRangeKey === ageRangeKey,
  );
  ...
  const priorityItem = useMemo(() => {
    return ageRangeMilestones
      .map((item) => ({
        item,
        status:
          records[item.key]?.status ?? ("not_started" as DevelopmentStatus),
      }))
  ...
  const concernItems = useMemo(() => {
    return ageRangeMilestones
      .filter((item) => records[item.key]?.status === "concern")
      .slice(0, 3);
  }, [ageRangeMilestones, records]);
  ...
  const recentHistory = useMemo(() => {
    return history
      .map((item) => ({
        ...item,
        milestone: getDevelopmentMilestone(item.itemKey),
      }))
      .filter((item) => item.milestone)
      .slice(0, 5);
  }, [history]);

  const changedHistory = recentHistory
    .filter(
      (item) => item.previousStatus && item.previousStatus !== item.status,
    )
    .slice(0, 3);
  const lastCheckedDate = history[0]?.createdAt
    ? new Date(history[0].createdAt)
    : null;
  const daysSinceLastCheck =
    lastCheckedDate && !Number.isNaN(lastCheckedDate.getTime())
      ? diffInDays(lastCheckedDate, today)
      : null;
  const nextCheckDate =
    lastCheckedDate && !Number.isNaN(lastCheckedDate.getTime())
      ? addDays(lastCheckedDate, 14)
      : addDays(today, 14);
  const nextCheckMessage =
  ...
  const recommendedRanges = useMemo(() => {
    ...
    if (currentRangeIndex > 0)
      keys.add(developmentAgeRanges[currentRangeIndex - 1].key);
    if (currentRangeIndex < developmentAgeRanges.length - 1)
      keys.add(developmentAgeRanges[currentRangeIndex + 1].key);
    return keys;
  ...
  async function saveStatus(itemKey: string, nextStatus: DevelopmentStatus) {
    ...
    setRecords((current) => ({
      ...current,
      [itemKey]: { ...current[itemKey], status: nextStatus },
    }));

    const response = await fetch(
      `/api/children/${selectedChild.id}/development-checks`,
      {
        method: "POST",
    ...
    if (!response?.ok) {
      setRecords((current) => ({
        ...current,
        [itemKey]: previous ?? { status: "not_started" },
      }));
    ...
    const data = (await response.json().catch(() => null)) as {
      log?: DevelopmentHistoryItem;
    } | null;
    if (data?.log)
      setHistory((current) =>
        [data.log as DevelopmentHistoryItem, ...current].slice(0, 80),
      );
    ...
    <main className="mt-page">
      <div className="mt-container max-w-xl space-y-3">
        <section className="mt-card p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <h1 className="mt-title-lg mt-3">
                {selectedName} 오늘 모습만 짧게 체크해요
              </h1>
              <p className="mt-text-main mt-2">
                {usesCorrectedAge
                  ? `실제 ${actualMonths}개월 · 참고 ${currentMonths}개월`
                  : `현재 약 ${currentMonths}개월`}{" "}
                · {nextCheckMessage}
              </p>
            </div>
            <Link
              href="/family"
              className="inline-flex shrink-0 items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-2 text-[11px] font-extrabold text-slate-700 active:scale-[0.98]"
            >
        ...
        <section
          ...
          aria-labelledby="development-child-title"
        >
        ...
        <TimelineLinkCard
          ...
          description={
            history.length
              ? `${history.length}개 기록을 시기별로 한눈에 보기`
              : "첫 체크를 저장하면 시기별 흐름을 볼 수 있어요"
          }
        />

        <section
          className="mt-card p-4"
          aria-labelledby="development-basic-title"
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2
                id="development-basic-title"
                className="text-[14px] font-extrabold text-slate-900"
              >
                확인할 기준
              </h2>
              <p className="mt-0.5 text-[12px] leading-5 text-slate-500">
                {range.label} · {domain.label}
              </p>
            </div>
            ...
            {developmentAgeRanges.map((item) => {
              ...
              return (
                <button
                  ...
                  data-mt-selected={isSelected ? "true" : undefined}
                  className={`mt-selectable-button shrink-0 rounded-full border px-3 py-2 text-[12px] font-extrabold transition active:scale-[0.98] ${
                    isSelected
            ...
            {developmentDomains.map((item) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={isSelected}
                  data-mt-selected={isSelected ? "true" : undefined}
                  className={`mt-selectable-button rounded-2xl border px-2 py-2 text-center text-[11px] font-extrabold transition active:scale-[0.98] ${
                    isSelected
                      ? "border-amber-200 bg-amber-50 text-amber-900"
                      : "border-slate-200 bg-white text-slate-600"
                  }`}
        ...
        {message ? (
          <p className="rounded-2xl bg-rose-50 px-4 py-3 text-[13px] font-bold leading-6 text-rose-700">
            {message}
          </p>
        ) : null}
        ...
        >
          <div className="border-b border-slate-100 px-4 py-3">
            ...
            <p className="mt-0.5 text-[12px] leading-5 text-slate-500">
              상태 버튼은 바로 저장돼요. 메모는 열어서 입력한 뒤 저장해 주세요.
            </p>
          </div>
                ...
                <article key={item.key} className="px-4 py-3.5">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-[13px] font-extrabold leading-5 text-slate-900">
                        {item.title}
                      </h3>
                      ...
                      <div className="mt-2.5 flex flex-wrap gap-1.5">
                        {statusOrder.map((status) => (
                          <StatusButton
                            ...
                            onClick={() => saveStatus(item.key, status)}
                          />
                        ))}
                      </div>
                      {selectedStatus ? (
                        <p className="mt-1.5 text-[11px] leading-4 text-slate-500">
                          {developmentStatusDescriptions[selectedStatus]}
                        </p>
                      ) : null}
                      <details className="mt-2 rounded-2xl border border-slate-100 bg-slate-50 px-3 py-2">
                        ...
                        <div className="mt-2 space-y-2 text-[12px] leading-5 text-slate-600">
                          <p>
                            <strong className="text-slate-800">볼 기준</strong>{" "}
                            · {item.observeTip}
                          </p>
                          <p>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <span
                                className={`text-[11px] font-bold ${savedNoteKey === item.key ? "text-emerald-600" : "text-slate-500"}`}
                              >
                                {savedNoteKey === item.key
                                  ? "메모를 저장했어요."
                                  : "메모는 저장 버튼을 눌러야 남아요."}
                              </span>
                              <button
        ...
        <section
          className="mt-card p-4"
          aria-labelledby="development-next-title"
        >
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="rounded-full bg-slate-50 px-2.5 py-1.5 text-[11px] font-extrabold text-slate-600">
                  다음 추천일 {formatKoreanShortDate(nextCheckDate)}
                </span>
                <Link
                  href={`/development-check/report?childId=${encodeURIComponent(selectedChild.id)}`}
                  className="rounded-full border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-extrabold text-slate-700 active:scale-[0.98]"
                >
                  상담용 기록
                </Link>
              </div>
                ...
                {changedHistory.length ? (
                  <ul className="mt-2 space-y-1.5 text-[12px] leading-5 text-slate-700">
                    {changedHistory.map((item) => (
                      <li key={`${item.itemKey}-${item.createdAt}`}>
                        • {item.milestone?.title}:{" "}
                        {item.previousStatus
                          ? developmentStatusLabels[item.previousStatus]
                          : "처음 기록"}{" "}
                  ...
                  <ul className="mt-2 space-y-1.5 text-[12px] leading-5 text-slate-700">
                    {recentHistory.map((item) => (
                      <li key={`${item.itemKey}-${item.createdAt}`}>
                        • {formatShortDate(item.observedAt ?? item.createdAt)} ·{" "}
                        {item.milestone?.title} ·{" "}
                        {developmentStatusLabels[item.status]}
                      </li>
                    ))}
                  ...
                  <p className="mt-2 text-[12px] leading-5 text-slate-600">
                    오늘 체크를 남기면 다음에 변화 흐름을 보여드릴게요.
                  </p>
                )}
                <p className="mt-2 text-[12px] leading-5 text-slate-600">
                  발달 체크는 진단표가 아니에요. 여러 항목이 반복해서 걱정되거나
                  이전에 하던 행동이 줄었다면 영유아검진이나 소아청소년과 상담
                  때 기록을 보여주세요.
                </p>
              </details>
            </div>
