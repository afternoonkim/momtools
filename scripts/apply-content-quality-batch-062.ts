...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-062-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-062 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 62;
  const resultMemo = batch.resultMemo ?? "batch-062 집 놀이·사회성·보행·반복 놀이·눈맞춤 문항 10개 정비 완료";

  console.log("batch-062 콘텐츠 품질 개선 반영을 시작합니다.");
