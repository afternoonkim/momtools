...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-058-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-058 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 58;
  const resultMemo = batch.resultMemo ?? "batch-058 침 흘림·성장곡선·손 사용·반복 말·걷기·키 성장·피로·스스로 먹기·몸 힘·두 돌 말수 문항 10개 정비 완료";

  console.log("batch-058 콘텐츠 품질 개선 반영을 시작합니다.");
