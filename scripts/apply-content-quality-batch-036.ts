...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-036-rewritten.json");
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 36;
  const resultMemo = batch.resultMemo ?? "batch-036 해열제 후 고열 동반 증상·두드러기 오르내림 관련 Q&A 10개 문항 본문, 카드 흐름, 검색 메타, 키워드, 상담 신호 정비 완료";

  console.log("batch-036 콘텐츠 품질 개선 반영을 시작합니다.");
