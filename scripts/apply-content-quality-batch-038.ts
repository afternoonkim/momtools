...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-038-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-038 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 38;
  const resultMemo = batch.resultMemo ?? "batch-038 약 복용 후 발진 후속 문항과 미열 3일 이상 문항 10개 정비 완료";

  console.log("batch-038 콘텐츠 품질 개선 반영을 시작합니다.");
