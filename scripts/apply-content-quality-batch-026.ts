...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-026-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-026 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 26`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-026 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-026 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/bug-bite-swollen-record");
  console.log("- http://localhost:3000/qna/health/bug-bite-swollen-with-other-symptoms");
  console.log("- http://localhost:3000/qna/health/burn-hot-water");
  console.log("- http://localhost:3000/qna/health/chapped-lips");
  console.log("- http://localhost:3000/qna/health/cold-hands-feet");
  console.log("- http://localhost:3000/qna/health/constipation-bloated-before-medicine");
  console.log("- http://localhost:3000/qna/health/constipation-bloated-er-signs");
  console.log("- http://localhost:3000/db-check/content-quality-progress");
  await prisma.$disconnect();
}

main().catch((error) => {
  console.error("batch-026 반영 실패");
  console.error(error);
  process.exit(1);
});
