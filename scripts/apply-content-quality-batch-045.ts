...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-045-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-045 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 45;
  const resultMemo = batch.resultMemo ?? "batch-045 야간 발열, 배변, 코피, 입안·눈·피부·수유 관련 건강 문항 10개 정비 완료";

  console.log("batch-045 콘텐츠 품질 개선 반영을 시작합니다.");
