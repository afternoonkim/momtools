...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-050-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-050 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 50;
  const resultMemo = batch.resultMemo ?? "batch-050 구토 뒤 수유 어려움, 쌕쌕거림, 하얀 변·노란 피부, 성장 식욕 문항 10개 정비 완료";

  console.log("batch-050 콘텐츠 품질 개선 반영을 시작합니다.");
