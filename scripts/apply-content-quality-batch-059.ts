...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-059-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-059 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 59;
  const resultMemo = batch.resultMemo ?? "batch-059 주먹 쥠·손 펄럭임·평발·납작머리·애착·대천문·넘어짐·질감 음식·어린이집 적응 문항 10개 정비 완료";

  console.log("batch-059 콘텐츠 품질 개선 반영을 시작합니다.");
