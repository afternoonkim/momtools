...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-063-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-063 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 63;
  const resultMemo = batch.resultMemo ?? "batch-063 언어 후퇴·활동량·식사·철분·발달표·입원 후 회복·구강 탐색·수면 문항 10개 정비 완료";

  console.log("batch-063 콘텐츠 품질 개선 반영을 시작합니다.");
