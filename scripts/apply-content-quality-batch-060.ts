...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-060-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-060 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 60;
  const resultMemo = batch.resultMemo ?? "batch-060 교정월령·영양·수면·성장곡선·측정 오차·성장 급등기·손 선호·계단 오르기·기질·터미타임 문항 10개 정비 완료";

  console.log("batch-060 콘텐츠 품질 개선 반영을 시작합니다.");
