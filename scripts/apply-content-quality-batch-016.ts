...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-016-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-016 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 16`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-016 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-016 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/baby-no-poop-but-gas");
  console.log("- http://localhost:3000/qna/health/baby-no-tears-crying");
  console.log("- http://localhost:3000/qna/health/baby-nostril-flaring");
  console.log("- http://localhost:3000/qna/health/baby-not-peeing-much");
  console.log("- http://localhost:3000/qna/health/baby-pink-eye-daycare");
  console.log("- http://localhost:3000/db-check/content-quality-progress");
  await prisma.$disconnect();
}

main().catch((error) => {
  console.error("batch-016 반영 실패");
  console.error(error);
  process.exit(1);
});
