...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-043-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-043 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 43;
  const resultMemo = batch.resultMemo ?? "batch-043 점액변 후속 문항 9개와 신생아 체온 38도 확인 문항 1개 정비 완료";

  console.log("batch-043 콘텐츠 품질 개선 반영을 시작합니다.");
