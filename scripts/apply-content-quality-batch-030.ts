...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-030-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-030 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 30`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-030 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-030 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-record");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-with-other-symptoms");
  console.log("- http://localhost:3000/qna/health/diarrhea-twice");
  console.log("- http://localhost:3000/qna/health/drooling-too-much");
  console.log("- http://localhost:3000/qna/health/droopy-weak");
  console.log("- http://localhost:3000/qna/health/dry-skin-itch");
  console.log("- http://localhost:3000/qna/health/ear-fever");
  console.log("- http://localhost:3000/qna/health/ear-pulling");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-before-medicine");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-daycare");
