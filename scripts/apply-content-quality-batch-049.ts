...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-049-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-049 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 49;
  const resultMemo = batch.resultMemo ?? "batch-049 예방접종 후 발열·보챔, 비타민 D, 구토 뒤 수유 어려움 건강 문항 10개 정비 완료";

  console.log("batch-049 콘텐츠 품질 개선 반영을 시작합니다.");
