...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-064-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-064 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 64;
  const resultMemo = batch.resultMemo ?? "batch-064 옹알이·컵 마시기·책 관심·식사 관심·배변훈련·관심 나누기·이름 반응·목가누기·가리키기·흉내 놀이 문항 10개 정비 완료";

  console.log("batch-064 콘텐츠 품질 개선 반영을 시작합니다.");
