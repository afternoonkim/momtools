...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-037-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-036 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 37;
  const resultMemo = batch.resultMemo ?? "batch-037 두드러기 동반 증상·음식 알레르기·발열 확인·철분·소변 감소·약 복용 후 발진 관련 Q&A 10개 문항 본문, 카드 흐름, 검색 메타, 키워드, 상담 신호 정비 완료";

  console.log("batch-037 콘텐츠 품질 개선 반영을 시작합니다.");
