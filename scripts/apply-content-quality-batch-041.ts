...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-041-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-041 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 41;
  const resultMemo = batch.resultMemo ?? "batch-041 입안 헐음·밥 거부 후속 문항과 콧물 색 변화 문항 10개 정비 완료";

  console.log("batch-041 콘텐츠 품질 개선 반영을 시작합니다.");
