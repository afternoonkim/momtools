...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-029-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-029 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 29`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-029 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-029 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/diaper-rash-red");
  console.log("- http://localhost:3000/qna/health/diarrhea-and-fever");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-before-medicine");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-daycare");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-er-signs");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-feeding");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-home-check");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-hospital");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-how-long");
  console.log("- http://localhost:3000/qna/health/diarrhea-less-pee-overnight");
