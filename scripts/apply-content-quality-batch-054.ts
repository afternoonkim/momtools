...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-054-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-054 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 54;
  const resultMemo = batch.resultMemo ?? "batch-054 도형·가상 놀이, 달리기, 끼적이기, 분류, 블록, 책장 넘기기, 숟가락 사용 발달 문항 10개 정비 완료";

  console.log("batch-054 콘텐츠 품질 개선 반영을 시작합니다.");
