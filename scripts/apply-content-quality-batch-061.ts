...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-061-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-061 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 61;
  const resultMemo = batch.resultMemo ?? "batch-061 머리둘레·목 가누기·키 성장·활동량·음식 머금기 문항 10개 정비 완료";

  console.log("batch-061 콘텐츠 품질 개선 반영을 시작합니다.");
