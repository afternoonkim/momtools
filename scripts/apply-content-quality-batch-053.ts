...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-053-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-053 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 53;
  const resultMemo = batch.resultMemo ?? "batch-053 몸짓 모방, 씹기, 지시 이해, 점프·공 차기, 색깔 맞추기 발달 문항 10개 정비 완료";

  console.log("batch-053 콘텐츠 품질 개선 반영을 시작합니다.");
