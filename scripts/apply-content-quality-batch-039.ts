...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-039-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-039 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 39;
  const resultMemo = batch.resultMemo ?? "batch-039 미열 3일 이상 후속 문항, 우유 반응, 모기 물림 붓기, 음식 후 입 주변 발진 문항 10개 정비 완료";

  console.log("batch-039 콘텐츠 품질 개선 반영을 시작합니다.");
