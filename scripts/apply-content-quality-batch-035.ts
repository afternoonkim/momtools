...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-035-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-035 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 35;
  const resultMemo = batch.resultMemo ?? "batch-035 감기 증상 없는 열·해열제 후 고열 반복 관련 Q&A 10개 문항 본문, 카드 흐름, 검색 메타, 키워드, 상담 신호 정비 완료";

  console.log("batch-035 콘텐츠 품질 개선 반영을 시작합니다.");
