...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-046-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-046 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 46;
  const resultMemo = batch.resultMemo ?? "batch-046 쌕쌕거림·거친 숨소리 관련 호흡 문항 10개 정비 완료";

  console.log("batch-046 콘텐츠 품질 개선 반영을 시작합니다.");
