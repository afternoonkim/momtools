...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-047-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-047 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 47;
  const resultMemo = batch.resultMemo ?? "batch-047 콧물·열성경련·수면·소변·복통·피부·눈·목 멍울 건강 문항 10개 정비 완료";

  console.log("batch-047 콘텐츠 품질 개선 반영을 시작합니다.");
