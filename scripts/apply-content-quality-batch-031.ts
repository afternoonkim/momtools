...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-031-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-031 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 31`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-031 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-031 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-er-signs");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-feeding");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-home-check");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-hospital");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-how-long");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-overnight");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-record");
  console.log("- http://localhost:3000/qna/health/ear-touch-cry-with-other-symptoms");
  console.log("- http://localhost:3000/qna/health/egg-allergy");
  console.log("- http://localhost:3000/qna/health/eye-discharge");
