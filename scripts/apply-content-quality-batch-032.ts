...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-032-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-032 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 32;
  const resultMemo = batch.resultMemo ?? "batch-032 눈곱·눈 충혈 관련 Q&A 10개 문항 정비 완료";

  console.log("batch-032 콘텐츠 품질 개선 반영을 시작합니다.");
