...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-057-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-057 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 57;
  const resultMemo = batch.resultMemo ?? "batch-057 모방·통 열기·가리키기·보여주기·손 바꾸기·빠이빠이·옷 입기·이유식 거부·침 흘림 문항 10개 정비 완료";

  console.log("batch-057 콘텐츠 품질 개선 반영을 시작합니다.");
