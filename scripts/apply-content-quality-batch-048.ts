...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-048-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-048 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 48;
  const resultMemo = batch.resultMemo ?? "batch-048 눈물·이앓이·차멀미·예방접종 후 발열과 보챔 건강 문항 10개 정비 완료";

  console.log("batch-048 콘텐츠 품질 개선 반영을 시작합니다.");
