...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-023-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-023 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 23`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-023 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-023 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/baby-vomiting-yellow-green");
  console.log("- http://localhost:3000/qna/health/baby-vomits-after-coughing");
  console.log("- http://localhost:3000/qna/health/baby-vomits-after-feeding");
  console.log("- http://localhost:3000/qna/health/baby-wheezing-after-cold");
  console.log("- http://localhost:3000/qna/health/baby-white-tongue");
  console.log("- http://localhost:3000/qna/health/bee-sting");
  console.log("- http://localhost:3000/db-check/content-quality-progress");
  await prisma.$disconnect();
}

main().catch((error) => {
  console.error("batch-023 반영 실패");
  console.error(error);
  process.exit(1);
});
