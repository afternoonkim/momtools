...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-056-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-056 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 56;
  const resultMemo = batch.resultMemo ?? "batch-056 시력·발달 상담 기록·발달 지연/후퇴 신호·공 던지기·감각 예민·책/보여주기·씹기 문항 10개 정비 완료";

  console.log("batch-056 콘텐츠 품질 개선 반영을 시작합니다.");
