...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-034-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-034 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 34;
  const resultMemo = batch.resultMemo ?? "batch-034 머리 부딪힘 뒤 졸림·배꼽 탈장 의심 관련 Q&A 10개 문항 정비 완료";

  console.log("batch-034 콘텐츠 품질 개선 반영을 시작합니다.");
