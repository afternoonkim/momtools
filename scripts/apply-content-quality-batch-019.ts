...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-019-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-019 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 19`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-019 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-019 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/baby-runny-nose-yellow-green");
  console.log("- http://localhost:3000/qna/health/baby-scalp-cradle-cap-worse");
  console.log("- http://localhost:3000/qna/health/baby-scratch-face-night");
  console.log("- http://localhost:3000/qna/health/baby-shakes-when-excited");
  console.log("- http://localhost:3000/qna/health/baby-sleep-startle-often");
  console.log("- http://localhost:3000/qna/health/baby-sleepy-after-vaccine");
  console.log("- http://localhost:3000/qna/health/baby-small-cut");
  console.log("- http://localhost:3000/db-check/content-quality-progress");
  await prisma.$disconnect();
}

main().catch((error) => {
  console.error("batch-019 반영 실패");
  console.error(error);
  process.exit(1);
});
