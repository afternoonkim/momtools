...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-051-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-051 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 51;
  const resultMemo = batch.resultMemo ?? "batch-051 집중 시간, 옹알이 시기, 옹알이 감소와 발달 상담 문항 10개 정비 완료";

  console.log("batch-051 콘텐츠 품질 개선 반영을 시작합니다.");
