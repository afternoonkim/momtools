...
async function loadUpdates(): Promise<ContentUpdate[]> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-027-rewritten.json");
...
async function markBatchItem(prisma: any, batchItemId: string, contentId: string) {
  try {
    ...
    if (columnSet.has("resultMemo")) assignments.push(`"resultMemo" = 'batch-027 페이지 전체 카드 점검 및 사용자 관점 리라이팅 완료'`);
...
async function markBatch(prisma: any) {
  try {
    ...
    if (assignments.length > 0) {
      await prisma.$executeRawUnsafe(
        `update "ContentQualityBatch" set ${assignments.join(", ")} where "batchNo" = 27`,
      );
    }
  } catch (error) {
...
async function main() {
  ...
  console.log("batch-027 콘텐츠 품질 개선 반영을 시작합니다.");
  ...
  console.log("");
  console.log("batch-027 반영 완료");
  console.log("확인 경로:");
  console.log("- http://localhost:3000/qna/health/constipation-bloated-hospital");
  console.log("- http://localhost:3000/qna/health/constipation-bloated-how-long");
  console.log("- http://localhost:3000/qna/health/constipation-bloated-overnight");
  console.log("- http://localhost:3000/qna/health/constipation-bloated-record");
  console.log("- http://localhost:3000/qna/health/constipation-bloated-with-other-symptoms");
  console.log("- http://localhost:3000/qna/health/constipation-hard-stool");
  console.log("- http://localhost:3000/qna/health/cough-at-night");
  console.log("- http://localhost:3000/qna/health/covid-baby");
  console.log("- http://localhost:3000/qna/health/croup-bark-cough");
  console.log("- http://localhost:3000/qna/health/dehydration-signs");
  console.log("- http://localhost:3000/db-check/content-quality-progress");
  await prisma.$disconnect();
}

main().catch((error) => {
  console.error("batch-027 반영 실패");
  console.error(error);
  process.exit(1);
});
