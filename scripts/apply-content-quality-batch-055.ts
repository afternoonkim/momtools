...
async function loadUpdates(): Promise<{ batch: RewrittenBatch["batch"]; updates: ContentUpdate[] }> {
  ...
  const filePath = path.join(__dirname, "../exports/content-quality/batch-055-rewritten.json");
  ...
  return {
    ...
    updates: parsed.items.map((item) => {
      ...
      return {
        ...
        resultMemo: item.resultMemo ?? `batch-055 페이지 정비 완료: ${content.path}`,
...
async function main() {
  ...
  const batchNo = batch.batchNo ?? 55;
  const resultMemo = batch.resultMemo ?? "batch-055 체중·활동량, 오르기, 손동작, 형제 비교, 교정월령, 귀 막기, 기기, 어린이집 피드백, 씹기, 말 발달과 청력 문항 10개 정비 완료";

  console.log("batch-055 콘텐츠 품질 개선 반영을 시작합니다.");
